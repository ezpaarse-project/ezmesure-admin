...
exports.handler = async function handler(argv) {
  ...
  if (verbose) {
    console.log(`Stopping harvest session ${harvestId} from ${config.ezmesure.baseUrl}`);
  }

  try {
    await harvestLib.stop(harvestId);
  } catch (error) {
    console.error(formatApiError(error));
    process.exit(1);
  }

  console.log(chalk.green(i18n.t('harvest.stop.success', { id: harvestId })));
