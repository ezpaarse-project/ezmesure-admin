...
module.exports = {
  findObjects: async (type, opts) => {
    ...
    let response;
    const url = (opts && opts.filter) ? `${config.kibanaUrl}/s/${opts.space}/api/saved_objects/_find` : `${config.kibanaUrl}/api/saved_objects/_find`;
    try {
      response = await instance.get(url, {
        params: {
          type,
          search_fields: (opts && opts.filter) ? 'title' : null,
          search: opts.filter || null,
        },
      });
    } catch (error) {
      logger.error(error);
