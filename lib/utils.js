 ...
 *
 * @returns {Promise<string | undefined>} The read text, or undefined if not available
 */
exports.readAllStdin = () => new Promise((resolve, reject) => {
 ...
 */
exports.readAllStdinAsJson = async () => {
  try {
    const data = await exports.readAllStdin();
    if (!data) { return undefined; }

    return JSON.parse(data);
  } catch (error) {
    console.error(`Couldn't read JSON from stdin: ${error}`);
    process.exit(0);
    return undefined;
  }
};
