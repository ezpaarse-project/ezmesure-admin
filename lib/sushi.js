...
module.exports = {
  ...
  findAll: async (institutionId) => client.search({
    ...
    body: {
      query: {
        bool: {
          ...
          must: [{
            match: {
              'sushi.institutionId': institutionId,
            },
          }],
        },
      },
  ...
  sushiTest: async (institution, options) => {
  ...
  importSushi: (docs) => {
    ...
    docs.forEach((sushiDoc) => {
      bulk.push({ index: { _index: config.institutionsIndex, _id: `${sushiDoc._id}` } });
      delete sushiDoc.doc._id;
      bulk.push({ type: sushiDoc.type, sushi: sushiDoc.doc });
    });
