...
module.exports = {
  getAll: (params = {}) => ezmesure.get('/sushi', { params: { size: 0, ...params } }),
  ...
  update: (sushiId, data) => ezmesure.patch(`/sushi/${sushiId}`, data),
  fetch: (sushiId) => ezmesure.post(`/sushi/${sushiId}_fetch`),
  getTasks: (sushiId, params = {}) => ezmesure.get(`/sushi/${sushiId}/tasks`, { params: { size: 0, ...params } }),
  getReports: (sushiId) => ezmesure.get(`/sushi/${sushiId}/reports`),
  downloadReport: (sushiId) => ezmesure.get(`/sushi/${sushiId}/report.json`),
  import: (data, opts) => ezmesure.post('/sushi/_import', data, opts),

  test: (data, params) => ezmesure.post('/sushi/_check_connection', data, { params, timeout: 0 }),
  testOne: (id, params) => ezmesure.post(`/sushi/${id}/_check_connection`, {}, { params, timeout: 0 }),
