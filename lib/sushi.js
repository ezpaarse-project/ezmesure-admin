  ...
  sushiTest: async (institution) => {
    ...
    try {
      ...
      if (error?.response?.data?.Exception) {
        return Promise.reject(new Error(`[Error#${error.response.data.Exception.Code}] ${error.response.data.Exception.Message}`));
      }
      if (error?.response?.data?.Severity) {
        return Promise.reject(new Error(`[Error#${error.response.data.Code}] ${error.response.data.Message}`));
      }
    ...
    if (!response) {
      return Promise.reject(new Error('Endpoint doesn\'t respond'));
    }
    ...
    if (response?.data?.Exception) {
      return Promise.reject(new Error(`[Error#${response.data.Exception.Code}] ${response.data.Exception.Message}`));
    }
    if (response?.data?.Severity) {
      return Promise.reject(new Error(`[Error#${response.data.Code}] ${response.data.Message}`));
    }
